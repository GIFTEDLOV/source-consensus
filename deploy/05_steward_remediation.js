...
// genlayer-js 0.24.0 defines FINALIZED as this exact wire-level enum string.
// Keep the deploy script dependency-free so the CLI can stage and execute it reproducibly.
export const TransactionStatus = Object.freeze({ FINALIZED: "FINALIZED" });
...
export const EXPECTED_ARTIFACT_SHA256 =
  "e0a732644683c8af6c15cdd781ef85c98eacddcb0febbec5df1fd6d1209796b9";
...
function fail(message) {
  throw new Error(`Steward remediation preflight failed: ${message}`);
}
...
export function buildDeployableCode() {
  ...
  assert(bytes.length === 51585, `deployable byte length is ${bytes.length}`);
...
export async function preTransactionGate(client) {
  ...
  assert(BigInt(balance) > 0n, "sender has no spendable balance");
...
export async function resolveFixture(client, address, sender) {
  ...
  writeFileSync(resolve(ROOT, "artifacts/steward-remediation-resolve-submitted.json"), JSON.stringify({ transaction: tx, address, sender }, null, 2) + "\n");
  ...
  assert(isDeepStrictEqual(parsedRecord.unavailable_source_indices, []), "unavailable bucket mismatch");
  assert(isDeepStrictEqual(parsedRecord.ambiguous_source_indices, []), "ambiguous bucket mismatch");
  assert(isDeepStrictEqual(parsedRecord.no_value_source_indices, []), "no-value bucket mismatch");
  const sources = await read(client, address, "get_sources");
  assert(sources.length === sourceUrls.length, "resolved source count mismatch");
  for (let index = 0; index < sources.length; index += 1) {
    assert(sources[index].source_index === index, `source order mismatch at ${index}`);
    assert(sources[index].state === "VALUE", `source ${index} state is ${sources[index].state}`);
    assert(sources[index].value === value, `source ${index} value is ${sources[index].value}`);
  }
  ...
  const proof = { tx, receipt, status, value, result, record, sources };
  writeFileSync(
    resolve(ROOT, "artifacts/steward-remediation-live-proof.json"),
    JSON.stringify(proof, (_, item) => typeof item === "bigint" ? item.toString() : item, 2) + "\n",
  );
  return proof;
}
...
export default async function main(client) {
  ...
  writeFileSync(resolve(ROOT, "artifacts/steward-remediation-deployment-submitted.json"), JSON.stringify({ transaction, sender: gate.sender, nonce: gate.nonce, balance: gate.balance }, null, 2) + "\n");
  ...
  writeFileSync(
    resolve(ROOT, "artifacts/steward-remediation-deployment-finalized.json"),
    JSON.stringify({ transaction, address, receipt }, (_, item) => typeof item === "bigint" ? item.toString() : item, 2) + "\n",
  );
  ...
  console.log("Steward remediation deployment and resolve finalized", { transaction, address, result });
}
